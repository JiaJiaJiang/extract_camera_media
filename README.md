...
> 本项目由 **DeepSeek AI (deepseek-v4-flash)** 编写。
...
- 每转码一个视频都校验转码目标能被 ffprobe 正确解析且至少有一个音频或视频轨道（校验失败则忽略，不复制不移动）
- 扫描时若音频/视频无法被 ffprobe 解析，则跳过处理（不转码不复制）
- 转码参数按编码器类型自动适配（nvenc / amf / qsv / libx 各编码器参数不同）
- 目标文件已存在时：转码到 `.processing` 临时文件，转码结果比已存在目标大且源文件更小时改为复制源文件，否则忽略；转码结果和源文件都比已存在目标小时用更小的那个替换
- 转码输出使用配置的容器格式（`container`，默认 `mp4`），源文件扩展名与容器不同时目标文件使用容器扩展名（如源 `.AVI` 转码后输出 `.mp4`）
- 同名不同后缀的文件（如 `video.AVI` 和 `video.MP4`）转码到同一目标时自动串行处理，避免并发写同一文件
- 错误文件（无法解析、转码结果无效）移到错误文件目录（如果指定）；所有处理过的正常文件移到已处理目录
